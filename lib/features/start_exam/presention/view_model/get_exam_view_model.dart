...
import 'package:exam_app_project/features/start_exam/domain/use_cases/get_exam_use_case.dart';
...
class GetExamViewModel extends Cubit<GetExamStates> {
  ...
  Future<BaseResponse<GetExamModel>> doIntent(GetExamEvents events ,String examId) => getExam(examId);

  Future<BaseResponse<GetExamModel>> getExam (String examId) async{
    emit(state.copyWith(getExamStateParam: BaseState<GetExamModel>(isLoading: true )));
    BaseResponse<GetExamModel> getExamResponse = await getExamUseCase.startExam(examId);
    switch(getExamResponse){
      ...
      case SuccessResponse<List<GetExamModel>>():
    }
