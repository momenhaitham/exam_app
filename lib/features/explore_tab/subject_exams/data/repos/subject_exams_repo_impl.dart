...
class SubjectExamsRepoImpl implements SubjectExamsRepoContract {
  ...
  @override
  Future<BaseResponse<SubjectExmasModel>> getSubjectExams(String? subject,String? token)async {
  try {
    BaseResponse<SubjectExamsDto> dataResponse = await dataSource.getSubjectExams(subject,token);
    switch (dataResponse) {
  ...
  Future<BaseResponse<SubjectExmasModel>> getAllExams(String? token) async{
    try {
    BaseResponse<SubjectExamsDto> dataResponse = await dataSource.getAllExams(token);
    switch (dataResponse) {
    case SuccessResponse<SubjectExamsDto>():
    return SuccessResponse<SubjectExmasModel>(data: dataResponse.data.toModel());
    case ErrorResponse<SubjectExamsDto>():
